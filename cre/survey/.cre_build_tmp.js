...
class SliceOffsetOutOfBoundsError extends BaseError3 {
  constructor({ offset, position, size }) {
    super(`Slice ${position === "start" ? "starting" : "ending"} at offset "${offset}" is out-of-bounds (size: ${size}).`, { name: "SliceOffsetOutOfBoundsError" });
  }
}

class SizeExceedsPaddingSizeError extends BaseError3 {
...
class IntegerOutOfRangeError extends BaseError3 {
  constructor({ max, min, signed, size, value }) {
    super(`Number "${value}" is not in safe ${size ? `${size * 8}-bit ${signed ? "signed" : "unsigned"} ` : ""}integer range ${max ? `(${min} to ${max})` : `(above ${min})`}`, { name: "IntegerOutOfRangeError" });
  }
}

class InvalidBytesBooleanError extends BaseError3 {
  constructor(bytes) {
...
class SizeOverflowError extends BaseError3 {
...
function prepareParam({ param, value }) {
  ...
  if (param.type.startsWith("uint") || param.type.startsWith("int")) {
    ...
    const [, , size = "256"] = integerRegex2.exec(param.type) ?? [];
...
class InvalidDecimalNumberError extends BaseError3 {
  constructor({ value }) {
...
var prepareReportRequestFromBytes = (payload, reportEncoder) => ({
...
var prepareReportRequest = (hexEncodedPayload, reportEncoder = EVM_DEFAULT_REPORT_ENCODER) => prepareReportRequestFromBytes(hexToBytes3(hexEncodedPayload), reportEncoder);
...
var assetSchema = objectType({
  symbol: stringType(),
  pair: stringType(),
  haircutBps: numberType().int().min(1).max(1e4),
  liquid: booleanType()
});
var configSchema = objectType({
  confidential: booleanType(),
  debug: booleanType(),
  chainSelectorName: stringType(),
  registryAddress: stringType(),
  apiBaseUrl: stringType(),
  holdBaseUrl: stringType(),
  holdSecretId: stringType(),
  priceMaxAgeSeconds: numberType().int().positive(),
  markTtlSeconds: numberType().int().positive(),
  sourceSetId: stringType(),
  workflowId: stringType(),
  gasLimit: stringType(),
  assets: arrayType(assetSchema).min(1)
});
var USD_SCALE = 100000000n;
var WAD = 1000000000000000000n;
var BPS = 10000n;
var VERDICT_INDETERMINATE = 0;
var VERDICT_ABOVE = 1;
var VERDICT_BELOW = 2;
var METRIC_OBSERVED_NET_ASSETS_USD = 0;
var METRIC_OBSERVED_LIQUID_USD = 1;
var COMPARATOR_GTE = 0;

class Indeterminate extends Error {
}
function usd1e8(amount, quote) {
  const units = parseUnits(amount, 18);
  if (units < 0n)
    throw new Indeterminate("negative position amount");
  return units * BigInt(quote.price) * USD_SCALE / (10n ** BigInt(quote.decimals) * WAD);
}
function valueHold(hold, assets, quotes, metricId) {
  const quoteFor = (symbol) => {
    const asset = assets.get(symbol);
    if (!asset)
      throw new Indeterminate("position in a non-allowlisted asset");
    const quote = quotes.get(asset.pair);
    if (!quote)
      throw new Indeterminate("no quote for an allowlisted asset");
    return { asset, quote };
  };
  let total = 0n;
  for (const p of hold.positions) {
    const { asset, quote } = quoteFor(p.symbol);
    if (metricId === METRIC_OBSERVED_LIQUID_USD && !asset.liquid)
      continue;
...
function evaluate(value, line) {
  if (line.comparator !== COMPARATOR_GTE)
    throw new Indeterminate("unsupported comparator");
...
var holdSchema = objectType({
  observedAt: numberType().int(),
  positions: arrayType(objectType({ symbol: stringType(), amount: stringType() })),
  liabilities: arrayType(objectType({ symbol: stringType(), amount: stringType() }))
});
function readLine(fetch, apiBaseUrl, subjectId, lineId) {
  const parsed = ladderSchema.safeParse(fetch({ url: `${apiBaseUrl}/api/ladder/${subjectId}`, method: "GET" }));
  if (!parsed.success)
    throw new Indeterminate("ladder unreadable");
  const rung = parsed.data.ladder.find((l) => l.lineId === lineId);
  if (!rung)
    throw new Indeterminate("line not on the subject's ladder");
  return { metricId: rung.metricId, comparator: 0, threshold: rung.threshold, publishBelow: rung.publishBelow };
}
function readHold(fetch, holdBaseUrl, subjectId, credential) {
  const parsed = holdSchema.safeParse(fetch({
    url: `${holdBaseUrl}/hold/${subjectId}`,
    method: "GET",
    headers: { Authorization: `Bearer ${credential}` }
  }));
  if (!parsed.success)
    throw new Indeterminate("hold unreadable");
  return parsed.data;
}
function readPrices(fetch, apiBaseUrl, pairs) {
  ...
  for (const pair of [...new Set(pairs)].sort()) {
    ...
    if (!parsed.success)
      throw new Indeterminate("price unreadable");
    quotes.set(parsed.data.pair, parsed.data);
  }
  return quotes;
}
var surveyRequested = parseAbiItem("event SurveyRequested(bytes32 indexed surveyId, bytes32 indexed subjectId, uint16 lineId, address indexed requester, uint64 requestedAt)");
function decodeRequest(event) {
  const decoded = decodeEventLog({
    abi: [surveyRequested],
    topics: event.topics.map((t) => bytesToHex3(t)),
    data: bytesToHex3(event.data)
  });
  const { surveyId, subjectId, lineId } = decoded.args;
  return { surveyId, subjectId, lineId: Number(lineId) };
}
var enclaveFetch = (runtime) => (req) => {
  const response = new ClientCapability2().sendRequest(runtime, {
    url: req.url,
    method: req.method,
    multiHeaders: req.headers ? Object.fromEntries(Object.entries(req.headers).map(([k, v]) => [k, { values: [v] }])) : undefined
  }).result();
  if (!ok(response))
    throw new Indeterminate("upstream request failed");
  return json(response);
};
var nodeFetch = (runtime) => (req) => {
  const response = new ClientCapability2().sendRequest(runtime, {
    url: req.url,
    method: req.method,
    multiHeaders: req.headers ? Object.fromEntries(Object.entries(req.headers).map(([k, v]) => [k, { values: [v] }])) : undefined
...
function assess(fetch, config, request, credential, now) {
...
function assessOnTheDon(runtime, request) {
  ...
  const perNode = (nodeRuntime, cred) => assess(nodeFetch(nodeRuntime), nodeRuntime.config, request, cred, now);
  return runtime.runInNodeMode(perNode, ConsensusAggregationByFields({ verdict: identical, asOf: median }))(credential).result();
}
function assessInEnclave(runtime, request) {
  const credential = runtime.getSecret({ id: runtime.config.holdSecretId }).result().value;
  const now = Math.floor(runtime.now().getTime() / 1000);
  return assess(enclaveFetch(runtime), runtime.config, request, credential, now);
}
var inEnclave = (runtime) => typeof runtime.usingTheDons === "function";
var survey = (runtime, event) => {
  const config = runtime.config;
  const request = decodeRequest(event);
  if (config.debug)
    runtime.log(`survey ${request.surveyId} line ${request.lineId}`);
  let outcome;
  try {
    ...
    if (!(e instanceof Indeterminate))
      throw e;
    if (config.debug)
      runtime.log(`indeterminate: ${e.message}`);
  ...
  const encodedPayload = encodeAbiParameters(parseAbiParameters("bytes32 surveyId, uint8 verdict, uint64 asOf, uint64 expiry, bytes32 sourceSetHash, bytes32 workflowId"), [
    request.surveyId,
    outcome.verdict,
    BigInt(outcome.asOf),
    BigInt(expiry),
  ...
  const txResult = new ClientCapability(network.chainSelector.selector).writeReport(donRuntime, {
  ...
  return bytesToHex3(txResult.txHash ?? new Uint8Array(32));
};
var initWorkflow = (config) => {
  const network = getNetwork({ chainFamily: "evm", chainSelectorName: config.chainSelectorName });
  if (!network)
    throw new Error(`unknown chain selector name: ${config.chainSelectorName}`);
  const trigger = new ClientCapability(network.chainSelector.selector).logTrigger(logTriggerConfig({
    addresses: [config.registryAddress],
    topics: [[toEventSelector(surveyRequested)]],
    confidence: "FINALIZED"
  }));
  return config.confidential ? [handlerInTee(trigger, survey, [{ tee: "nitro", regions: ["us-west-2"] }])] : [handler(trigger, survey)];
};
async function main() {
