...
5. **Fill the two `REPLACE_ME` fields** in each `recipes/*.recipe.json`: every `gateway_slug`
   ...
   Bazantic derives the handle from `name`; you do not choose it. Created and **published**
   2026-09-13: `request-mark` and `underwrite-counterparty`, model `anthropic/claude-sonnet-4.6`.
   Both gateways price every method at $0.01 USDC and settle on **Base** (the 402 body says
   `network: base`); flip sandbox mode on in the dashboard to settle test USDC on Base Sepolia
   instead, and create the grant on the same network.
